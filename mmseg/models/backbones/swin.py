...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W,swinresult):
        #swinresult列表用来存储四个阶段的特征图
...
class SwinTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None,pretrained1=None):
        if isinstance(pretrained, str):
            ...
            for k, v in list(state_dict.items()):
                name = k[9:]  # remove `backbone.`，只取backbone.0.weights的后面几位
                model_dict['pvt.'+name] = v
            self.load_state_dict(model_dict, strict=False)
            # load_checkpoint(self, pretrained, map_location='cpu', strict=False, logger=logger)
        if isinstance(pretrained1, str):
            logger = get_root_logger()
            state_dict = torch.load(pretrained1)  # 加载预训练模型
            model_dict = self.state_dict()
    ...
    def forward(self, x):
        # x: [B, L, C]
        swinresult=[]
        pvtresult = self.pvt(x) # 4个list
        x, H, W = self.patch_embed(x)
        x = self.pos_drop(x)

        for layer in self.layers:
            x, H, W,swinresult = layer(x, H, W,swinresult)
