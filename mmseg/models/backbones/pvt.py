...
def svd_denoise(img,alpha):
    h, w = img.shape[:2]  # 获取图像 高和宽
    flag = 0
    if h > w:
        img = np.transpose(img)  # 非长方阵，转置
        h, w = img.shape[:2]  # 获取图像 高和宽
        flag = 1
    u, sigma, vt = la.svd(img)  # SVD分解
    h1=int(h*alpha) #取前10%的奇异值重构图像
    ...
    vt1[:,:]=vt[:h1,:]
    if flag == 1:
        return np.transpose(u1.dot(sigma1).dot(vt1))
    else:
