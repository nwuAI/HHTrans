...
#空间转换块spatial feature transform
class SFTLayer(nn.Module):
    def __init__(self, head_channels):
        super(SFTLayer, self).__init__()
        # self.trans = nn.Conv2d(128,32,1)
    ...
    def forward(self, local_features, global_features):
        ...
        # global_features=self.trans(global_features)
        scale = self.SFT_scale_conv1(F.relu(self.SFT_scale_conv0(global_features),inplace=True)) #torch.Size([1, 128, 320, 480])
        shift = self.SFT_shift_conv1(F.relu(self.SFT_shift_conv0(global_features),inplace=True))#torch.Size([1, 128, 320, 480])
...
class Local8x8_fuse_head(BaseDecodeHead):
    ...
    def forward(self, local_features, global_features):
        fuse_features = self.SFT_head(local_features, global_features)#[1,1,320,480]
