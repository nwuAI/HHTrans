...
class BIMLAHead(nn.Module):
    def __init__(self):
        ...
        self.head2 = nn.Sequential(nn.ConvTranspose2d(256, 128, 8, stride=4, padding=2,bias=False), nn.BatchNorm2d(128), nn.ReLU())
        self.head3 = nn.Sequential(nn.ConvTranspose2d(512, 128, 16, stride=8, padding=4,bias=False), nn.BatchNorm2d(128), nn.ReLU())
        self.head4 = nn.Sequential(nn.ConvTranspose2d(1024, 128, 4, stride=2, padding=1,bias=False), nn.BatchNorm2d(128), nn.ReLU(),
                                   nn.ConvTranspose2d(128, 128, 16, stride=8, padding=4,bias=False), nn.BatchNorm2d(128), nn.ReLU())
        self.head5 = nn.Sequential(nn.ConvTranspose2d(2048, 128, 4, stride=2, padding=1,bias=False), nn.BatchNorm2d(128), nn.ReLU(),
...
class PVTResnet_Head2(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, mla_channels=256, mlahead_channels=128,
        ...
        super(PVTResnet_Head2, self).__init__(**kwargs)
