...
class EncoderDecoder_Pvtcnn(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 global_cfg=None,
                 pretrained=None):
        super(EncoderDecoder_Pvtcnn, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        super(EncoderDecoder_Pvtcnn, self).init_weights(pretrained)
    ...
    def extract_feat(self, img,gfeature):
        """Extract features from images."""
        x = self.backbone(img,gfeature)
        if self.with_neck:
            x = self.neck(x)
        return x

    def encode_decode(self, img,gfeature, img_metas):
        """Encode images with backbone into a tuple list."""
        x = self.extract_feat(img,gfeature)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        self.global_model.eval()
        with torch.no_grad():
            global_features = self.global_model.inference_seg_logits_feature(img).detach()#截断，防止训练
        # batch_size, _, h_img, w_img = img.size()
        # h_encode = w_encode = int (20 * (h_img/self.backbone.img_size))#40
        # self.h_crop = self.w_crop = self.h_stride = self.w_stride = self.backbone.img_size
        # h_grids = max(h_img - self.h_crop + self.h_stride - 1, 0) // self.h_stride + 1#取整除得到1
        # w_grids = max(w_img - self.w_crop + self.w_stride - 1, 0) // self.w_stride + 1
        # preds1 = img.new_zeros((batch_size, 256, h_encode, w_encode))#[1,256,40,40]
        ...
        # count_mat = img.new_zeros((batch_size, 1, h_encode, w_encode))#我觉得是用来判断是否有空余的空间
        ...
        #         crop_seg_logit = self.extract_feat(crop_img)#经过文章中的decoder部分得到的八个输出
        ...
        x = self.extract_feat(img,global_features)
        losses = dict()
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                global_features = self.global_model.inference_seg_logits_feature(crop_img).detach()
                crop_seg_logit = self.encode_decode(crop_img,global_features, img_meta)  # torch.Size([1, 1, 320, 320])
