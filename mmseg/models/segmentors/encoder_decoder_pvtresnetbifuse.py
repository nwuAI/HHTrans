...
class EncoderDecoder_PvtBifusion(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 global_cfg=None,
                 pretrained=None):
        super(EncoderDecoder_PvtBifusion, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        super(EncoderDecoder_PvtBifusion, self).init_weights(pretrained)
    ...
    def encode_decode1(self, img,gfeature, img_metas):
        """Encode images with backbone into a tuple list."""
        x = self.extract_feat(img)
        out=self._decode_head_forward_test(x,gfeature, img_metas)
    ...
    # 更改过，需要传入第一阶段训练好的参数
    def _decode_head_forward_train(self, x,global_features, img_metas, gt_semantic_seg):
        """Run forward function and calculate loss for decode head in training.
           Generate the LOCAL FEATURE
        """
        losses = dict()
        loss_decode, local_features = self.decode_head.forward_train_with_local_features1(x,global_features, img_metas,
                                                     gt_semantic_seg,
                                                     self.train_cfg)
    ...
    # 更改过，需要传入第一阶段训练好的参数
    def _decode_head_forward_test(self, x,global_features, img_metas):
        """Run forward function and calculate loss for decode head in
        inference."""
        seg_logits, local_feature = self.decode_head.fuse_forward_test(x,global_features)
        return seg_logits
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        with torch.no_grad():
            ...
            # global_features = self.global_model.inference_global_feature(img)#截断，防止训练
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                global_features = self.global_model.inference_global_feature(crop_img)
...
if __name__ == '__main__':
    model = EncoderDecoder_PvtBifusion()
