...
class EncoderDecoderConformer(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(EncoderDecoderConformer, self).__init__()
        self.backbone = builder.build_backbone(backbone)
        if neck is not None:
            self.neck = builder.build_neck(neck)
        self._init_decode_head(decode_head)
        self._init_auxiliary_head(auxiliary_head)
        self._init_trans_head(trans_head)
    ...
    def _init_trans_head(self, fuse_head):
        """Initialize ``fuse_head``"""
        self.trans_head = builder.build_head(fuse_head)
    # def _init_transdecode(self,decoder_head):
    ...
    def init_weights(self, pretrained=None):
        ...
        super(EncoderDecoderConformer, self).init_weights(pretrained)
    ...
    def encode_decode(self, img, img_metas):
        ...
        out = self._transform_head_forward_test(x, img_metas)
        out = resize(
            input=out,
            size=img.shape[2:],
            mode='bilinear',
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg):
        ...
        losses.update(add_prefix(loss_decode, 'cnn_edge'))
        return losses
    ...
    def _transform_head_forward_test(self, x, img_metas):
        """Run forward function and calculate loss for decode head in
        inference."""
        seg_logits,_ = self.trans_head.forward_test2_1(x, img_metas, self.test_cfg)
        return seg_logits
    ...
    #transformer部分的损失
    def _trans_head_forward_train(self, x, img_metas, gt_semantic_seg):#fuse模块的前向传播
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        loss_fuse, _, _ = self.trans_head.forward_train2_1(x, img_metas,
                                                     gt_semantic_seg,
                                                     self.train_cfg)

        losses.update(add_prefix(loss_fuse, 'transformer_edge'))
        return losses
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        #transformer部分的损失
        loss_transdecode = self._trans_head_forward_train(x, img_metas,
                                                      gt_semantic_seg)
        losses.update(loss_transdecode)
