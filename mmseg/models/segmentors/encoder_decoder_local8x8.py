...
class EncoderDecoder_LOCAL8x8(BaseSegmentor):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        self.global_model.eval()
        with torch.no_grad():
            global_features = self.global_model.inference_global_feature(img).detach()#截断，防止训练
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        """
        batch_size, _, h_img, w_img = img.size()
        img_crop = img[:, :, 0:h_img - 1, 0:w_img - 1]   # for BSDS500 torch.Size([1, 3, 320, 480])
        #img_crop = img[:, :, 0:h_img - 1, :]   # for NYUD 我觉得是要裁剪成偶数
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                x1 = max(x2 - w_crop, 0)
                crop_img = img[:, :, y1:y2, x1:x2]
                crop_seg_logit = self.encode_decode(crop_img, img_meta)# 8个数组，每一个都是torch.Size([1, 256, 20, 20])
        ...
        x_crop = (preds1, preds2, preds3, preds4, preds5, preds6, preds7, preds8) #8个数组，每一个都是torch.Size([1, 256, 40, 60])
